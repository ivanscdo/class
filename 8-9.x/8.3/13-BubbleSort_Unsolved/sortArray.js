...
// PSEUDOCODE
// take array;
// find length: take length of array minus second index since index 1;
// check first two pairs: index 0 and index 1;
// is index 0 greater than index 1? if yes, swap index, if no check index 1 and index 2;
// repeat check: is index 1 greater than index 2? if yes, swap index, if no check index 2 and index 3;
// repeat check: is index 2 greater than index 3? if yes, swap index, if no check index 3 and index 4;
// repeat check: is index 3 greater than index 4? if yes, swap index, if no check index 4 and index 5;
// get to end and repeat
// if no more swaps, then array in order
...
for (let i = 0; i < unsortedArr.length; i++) {
  ...
  if (unsortedArr[i] > unsortedArr[i + 1]) {
    ...
    var indexSwap = unsortedArr[i];
    unsortedArr[i] = unsortedArr[i + 1];
    unsortedArr[i + 1] = indexSwap;

    console.log(unsortedArr[i], unsortedArr[i + 1])

    
  //END OF: if (unsortedArr[i] > unsortedArr[i + 1]) {
  } else if (unsortedArr[i] < unsortedArr[i + 1]) {
    

  }

//END OF: for (let i = 0; i < unsortedArr.length; i++) {
}
